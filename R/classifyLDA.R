## Function to search by cliques (groups of genes) of classifiers, using
## exaustive search.
##
...
##             sortBy   -> order the good classifiers by 'cv' or 'svd'
##
## Gustavo Esteves
...
classifyLDA <- function(obj=NULL, sLabelID="Classification", facToClass=NULL,
gNameID="GeneName", geneGrp=1, path=NULL, nGenes=3, sortBy="cv") {
    ...
    if(!is.null(geneGrp) & !is.null(path))
        stop("You must specify only one of geneGrp or path, or leave
        both NULL.")
    
    if(!(sortBy %in% c("cv", "svd")))
        stop("The argument 'sortBy' must be 'cv' or 'svd'.")
    
    
    ## Defining 1 additional function to do more iterations
    oneMoreIter <- function(tab, idxAnt, sortBy) {
